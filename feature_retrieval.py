...
def load_queries(config):
    ...
    # Apply MedCorpus filtering here
    if config.dataset_type == "medcorpus":
        original_count = len(queries)
        queries = [q for q in queries if str(q.query_id).endswith("_3")]
        filtered_count = len(queries)
        logger.info(f"MedCorpus dataset detected for retrieval. Filtering for turn 3 queries: Kept {filtered_count}/{original_count} queries.")
        if filtered_count == 0 and original_count > 0:
             logger.warning("No turn 3 queries found for MedCorpus retrieval. Check query IDs.")
    return queries
